...
@php
    $baseClasses = 'p-4 rounded-lg border-l-4 flex items-start gap-3';
    ...
    $classes = trim("{$baseClasses} {$bgColor} {$borderColor}");
@endphp
...
<div {{ $attributes->merge(['class' => $classes]) }} role="alert">
    ...
    <div class="flex-1">
        @if($title)
            <p class="font-semibold {{ $textColor }} mb-1">{{ $title }}</p>
        @endif
    ...
    @if($dismissible)
        <button 
            type="button"
            class="flex-shrink-0 ml-auto {{ $textColor }} hover:opacity-75 transition-opacity"
